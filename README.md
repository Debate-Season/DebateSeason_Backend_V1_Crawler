...
3. 크롤링 작업 중에 발생한 Error에 대해서, throw 대신 -> log와 Sentry.io로 에러 전송. </br>
  : throw를 던질 경우, 나머지 정상적인 데이터를 못 가져오고 중지됨.
...
## 5-1. 최적화 작업 1 - Throttling.
  ...
  효과
  : 의도적인 성능 저하로 크롤링에 사용되는 시간은 늘어났지만, 다운그레이드된 환경에서도 크롤러가 안정적으로 작동함.

## 5-2. 최적화 작업 2 - 작업 큐 6개 -> 1개로 감소.
  : 기존 6개의 커뮤니티 각각에 대해서 @Scheduled를 등록함. 따라서, 6개의 @Scheduled 작업을 1vCPU로 처리하기에 상당히 비용이 크다고 판단함. </br>
  따라서, 1개의 @Scheduled 작업에 6개의 커뮤니티를 반복문으로 가져오도록 하되, 1개의 작업을 마친 후 Thread.sleep을 적용. 
